...
export const projectsAPI = {
  ...
  reports: {
    ...
    getReports: async (projectId: number) => {
      const response = await axios.get(`${API_URL}/${projectId}/reports`);
      return response.data.data;
    },

    // Add report
    addReport: async (projectId: number, reportType: 'daily' | 'weekly' | 'monthly', data: any) => {
      const response = await axios.post(`${API_URL}/${projectId}/reports/${reportType}`, data);
      return response.data.data;
    },

    // Update report
    updateReport: async (reportId: number, data: any) => {
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/reports/${reportId}`, data);
      return response.data.data;
    },
