...
export const apiKeyApi = {
  ...
  getApiKey: async (): Promise<ApiKeyResponse> => {
    const response = await axios.get(`${API_BASE_URL}/user/api-key`);
  ...
  saveApiKey: async (apiKey: string): Promise<{ message: string; user_id: number }> => {
    const response = await axios.post(`${API_BASE_URL}/user/api-key`, { api_key: apiKey });
    const data = response.data.data;
    // Also save to localStorage for backward compatibility
  ...
  deleteApiKey: async (): Promise<{ message: string; user_id: number }> => {
    const response = await axios.delete(`${API_BASE_URL}/user/api-key`);
    const data = response.data.data;
