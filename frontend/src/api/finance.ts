...
export const financeAPI = {
  ...
  getFinanceByType: async (type: 'income' | 'expense'): Promise<FinanceEntry[]> => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/filter/type?type=${type}`);
  ...
  getPaginated: async (params: PaginationParams): Promise<PaginatedResponse<FinanceEntry>> => {
    ...
    const response = await axios.get(`${API_URL}/paginated?${queryParams}`);
  ...
  getFinanceByTypePaginated: async (
    ...
    const response = await axios.get(`${API_URL}/filter/type?${queryParams}`);
    return response.data;
  },
  ...
  filterByDateRange: async (startDate: string, endDate: string): Promise<FinanceEntry[]> => {
    const response = await axios.get(`${API_URL}/filter/date-range?start_date=${startDate}&end_date=${endDate}`);
    return response.data?.data || [];
  },

  filterByAmountRange: async (minAmount: number, maxAmount: number): Promise<FinanceEntry[]> => {
    const response = await axios.get(`${API_URL}/filter/amount-range?min_amount=${minAmount}&max_amount=${maxAmount}`);
    return response.data?.data || [];
  },

  filterByCategory: async (category: string): Promise<FinanceEntry[]> => {
    const response = await axios.get(`${API_URL}/filter/category?category=${category}`);
    return response.data?.data || [];
  },

  filterByStatus: async (status: 'Paid' | 'Unpaid'): Promise<FinanceEntry[]> => {
    const response = await axios.get(`${API_URL}/filter/status?status=${status}`);
    return response.data?.data || [];
  },
};